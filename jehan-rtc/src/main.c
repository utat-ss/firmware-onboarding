 ...
 * Copyright (c) 2025 Jehan Billimoria
 *
 */
...
int main(void) {
    ...
    const struct device *rtc = DEVICE_DT_GET(DT_ALIAS(rtc));//tells zephyr to access the RTC device labelled as RTC_0 in devicetree
    if (rtc == NULL) {
        printk("Device binding failed\n");
        return 0;
    }
    ...
    while(1) {
        ...
        printk("%d, %d, %d, %d:%d:%d\n", timestamp.tm_year, timestamp.tm_mon + 1, timestamp.tm_mday, timestamp.tm_hour, timestamp.tm_min, timestamp.tm_sec);//logs timestamp
        k_msleep(800);
    }
}
