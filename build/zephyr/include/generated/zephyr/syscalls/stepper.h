...
extern int z_impl_stepper_set_callback(const struct device * dev, stepper_event_callback_t callback, void * user_data);

__pinned_func
static inline int stepper_set_callback(const struct device * dev, stepper_event_callback_t callback, void * user_data)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		...
		return (int) arch_syscall_invoke3(parm0.x, parm1.x, parm2.x, K_SYSCALL_STEPPER_SET_CALLBACK);
	}
#endif
	compiler_barrier();
	return z_impl_stepper_set_callback(dev, callback, user_data);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define stepper_set_callback(dev, callback, user_data) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_STEPPER_SET_CALLBACK, stepper_set_callback, dev, callback, user_data); 	syscall__retval = stepper_set_callback(dev, callback, user_data); 	sys_port_trace_syscall_exit(K_SYSCALL_STEPPER_SET_CALLBACK, stepper_set_callback, dev, callback, user_data, syscall__retval); 	syscall__retval; })
#endif
#endif
